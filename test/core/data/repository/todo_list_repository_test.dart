...
void main(){
  ...
  group('ToDo repository', (){
    final taskLocalModel = [TaskLocalModel(id: '1', text: 'getAll', importance: 'basic', done: false, lastUpdatedBy: 'test', changedAt: DateTime.utc(2023, 01, 02), createdAt: DateTime.utc(2023, 01, 01))];
    final taskApiModel = [TaskApiModel(id: '1', text: 'getAll', importance: 'basic', done: false, lastUpdatedBy: 'test', changedAt: DateTime.utc(2023, 01, 02), createdAt: DateTime.utc(2023, 01, 01))];
    ...
    when(()=> taskLocalDataSourceMock.createTaskToCache(any())).thenAnswer((_) async {});
      ...
      test('должен возвращать CacheFailure при ошибке Local Datasource', () async{
        //arrange
    ...
    group('и его метод getAllTasks', (){
      test('должен возвращать список задач из Local при отсутствии интернета', () async{
        //arrange
        when(()=>networkStatusMock.isOnline).thenReturn(false);
        when(()=>taskLocalDataSourceMock.getAllTasksFromCache()).thenAnswer((_) async => localResult);
      ...
      test('должен обновлять список задач в api (синхронизироваться) при наличии* интернета', () async{
        //arrange
        when(() => networkStatusMock.isOnline).thenReturn(true);
        when(() => taskLocalDataSourceMock.getAllTasksFromCache()).thenAnswer((_) async => localResult);
        when(() => taskRemoteDataSourceMock.getAllTasks()).thenAnswer((_) async => apiResult);
        when(() => taskRemoteDataSourceMock.updateAllTasks(any(), any())).thenAnswer((_) async => apiResult);
        ...
        verify(() => taskLocalDataSourceMock.updateLocalRevision(apiResult.apiRevision)).called(1);
        verify(() => taskLocalDataSourceMock.getAllTasksFromCache()).called(1);
        verify(() => taskRemoteDataSourceMock.updateAllTasks(taskApiModel, apiResult.apiRevision)).called(1);
      });

      test('должен возвращать CacheFailure, если случится CacheException', () async{
        //arrange
        when(() => networkStatusMock.isOnline).thenReturn(false);
        when(() => taskLocalDataSourceMock.getAllTasksFromCache()).thenThrow(CacheException('cache exception'));
      ...
      test('должен возвращать ServerFailure, если api выбросит ошибку', () async{
        //arrange
        when(() => networkStatusMock.isOnline).thenReturn(true);
        when(() => taskRemoteDataSourceMock.getAllTasks()).thenThrow(ServerException('server exception'));
      ...
      test('должен возвращать Failure при ошибке репозитория', () async{
        //arrange
        when(() => networkStatusMock.isOnline).thenReturn(true);
        when(() => toDoListRepositoryMock.getAllTasks()).thenThrow(Exception('repo exception'));

        //act
        final result = await toDoListRepository.getAllTasks();

        //assert
        expect(result, equals(Left(Failure('repo exception'))));
      });
    });

    group('и его метод updateAllTasks', (){
      final taskLocalModel = [TaskLocalModel(id: '1', text: 'updateAll', importance: 'basic', done: false, lastUpdatedBy: 'test', changedAt: DateTime.utc(2023, 01, 02), createdAt: DateTime.utc(2023, 01, 01))];
      final taskLocalResult = TaskLocalModelWithRevision(listTasks: taskLocalModel, localRevision: 1);
      final taskEntity = [TaskEntity(id: '1', text: 'updateAll', done: false, importance: 'basic', lastUpdatedBy: 'test', changedAt: DateTime.utc(2023, 01, 02), createdAt: DateTime.utc(2023, 01, 01))];
      test('должен обновлять Local задачи на новые', () async{
        //arrange
        when(() => taskLocalDataSourceMock.updateAllTasksToCache(taskLocalModel)).thenAnswer((_) async => taskLocalResult);
      ...
      test('должен возвращать CacheFailure при ошибке Local', ()async{
        //arrange
        when(() => taskLocalDataSourceMock.updateAllTasksToCache(taskLocalModel)).thenThrow(CacheException('cache exception'));

        //act
        final result = await toDoListRepository.updateAllTasks(taskEntity);

        //assert
        expect(result, equals(Left(CacheFailure('cache exception'))));
      });

      test('должен возвращать Failure при ошибке самого репозитория', ()async{
        //arrange
        when(() => toDoListRepositoryMock.updateAllTasks(taskEntity)).thenThrow(Exception('repo exception'));
    ...
    group('и его метод deleteTask', (){
      test('должен возвращать Right при успешном удалении из Local', () async{
        //arrange
        when(()=> taskLocalDataSourceMock.deleteExactTaskFromCache(taskLocalModel.first)).thenAnswer((_) async {});
        //act
      ...
      test('должен возвращать CacheFailure при ошибке удаления в Local', ()async{
        //arrange
        when(()=>taskLocalDataSourceMock.deleteExactTaskFromCache(taskLocalModel.first)).thenThrow(CacheException('local error'));

        //act
        final result = await toDoListRepository.deleteTask(taskEntities.first);

        //assert
        expect(result, equals(Left(CacheFailure('local error'))));
      });

      test('должен возвращать Failure при ошибке репо', () async{
        //arrange
        when(()=>toDoListRepositoryMock.deleteTask(taskEntities.first)).thenThrow(Exception('repo error'));

        //act
        final result = await toDoListRepository.deleteTask(taskEntities.first);

        //assert
        expect(result, Left(Failure('repo error')));
      });
    });
    ...
    group('и его метод getExactTaak', (){
      test('должен вернуть запрошенную задачу из Local', ()async{
        //arrange
        when(()=>taskLocalDataSourceMock.getExactTaskFromCache(taskLocalModel.first)).thenAnswer((_) async => localResult);

        //act
        final result = await toDoListRepository.getExactTask(taskEntities.first);

        //assert
        expect(result, isA<Right>());
        expect(result.fold((failure) => null, (task) => task), isA<TaskEntity>());
      });

      test('должен возвращать CacheFailure при ошибке Local', ()async{
        //arrange
        when(()=>taskLocalDataSourceMock.getExactTaskFromCache(taskLocalModel.first)).thenThrow(CacheException('local error'));
      ...
      test('должен возвращать Failure при ошибке репо', ()async{
        //arrange
        when(()=>toDoListRepositoryMock.getExactTask(taskEntities.first)).thenThrow(Exception('repo error'));
