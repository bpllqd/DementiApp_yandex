...
class ToDoCreateBloc extends Bloc<ToDoCreateEvent, ToDoCreateState> {
  ...
  Future<void> _createTask(ToDoCreateNewEvent event,Emitter<ToDoCreateState> emit,) async {
    emit(ToDoCreateLoadingState());
    DementiappLogger.infoLog('Loading...');
    const Duration(seconds: 2);

    emit(ToDoCreateSuccessState(task: event.task, hasDate: event.task.date != null));
    DementiappLogger.infoLog('Task has been created!');
  }

  Future<void> _deleteTask(ToDoCreateDeleteEvent event, Emitter<ToDoCreateState> emit,) async {
    emit(ToDoCreateLoadingState());
    DementiappLogger.infoLog('Loading...');    
    const Duration(seconds: 2);
  ...
  Future<void> _switchDate(ToDoCreateSwitchDateEvent event, Emitter<ToDoCreateState> emit,) async {
    emit(ToDoCreateSuccessState(task: event.task, hasDate: event.task!.date == null ? false : true));
